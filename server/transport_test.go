...
func newServer(t *testing.T, whois func(context.Context, string) (*apitype.WhoIsResponse, error)) *httptest.Server {
	t.Helper()
...
func TestTransportGuards(t *testing.T) {
	hs := newServer(t, whoIsRule(t, &acl.Rule{Action: []acl.Action{acl.ActionGet}, Secret: []acl.Secret{"*"}}))
	hc := hs.Client() // starts the in-memory network and populates hs.URL

	do := func(method string, headers map[string]string) int {
		req, err := http.NewRequest(method, hs.URL+"/api/get", strings.NewReader(`{"name":"s"}`))
		require.NoError(t, err)
...
func TestIdentityUnavailableIs503(t *testing.T) {
	hs := newServer(t, func(context.Context, string) (*apitype.WhoIsResponse, error) {
		return nil, errors.New("localapi: tailscaled restarting")
	})
	require.Equal(t, http.StatusServiceUnavailable, postFull(t, hs))
}
...
func postFull(t *testing.T, hs *httptest.Server) int {
	t.Helper()

	hc := hs.Client() // starts the in-memory network and populates hs.URL
	req, err := http.NewRequest(http.MethodPost, hs.URL+"/api/get", strings.NewReader(`{"name":"s"}`))
	require.NoError(t, err)
